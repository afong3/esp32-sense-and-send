...
/************************************************************/
/*  VL53L7CX ULD motion indicator with detection thresholds */
/************************************************************/
/*
 ...
 * the VL53L7CX interrupt is raised on GPIO1. This is the only way to use detection thresholds.
 */
/*
int WaitForL5Interrupt(VL53L7CX_Configuration * pDev) {
...
*/
extern int WaitForL5Interrupt(VL53L7CX_Configuration * pDev);
extern int IntCount;
...
int example11(void)
{
	...
	*/
	...
	status = vl53l7cx_motion_indicator_init(&Dev, &motion_config, VL53L7CX_RESOLUTION_8X8);
	if(status)
	{
	...
	status = vl53l7cx_set_integration_time_ms(&Dev, 10);
	...
	for(i = 0; i < 64; i++){
		...
		thresholds[i].measurement = VL53L7CX_MOTION_INDICATOR;
		thresholds[i].type = VL53L7CX_GREATER_THAN_MAX_CHECKER;
	...
	thresholds[63].zone_num = VL53L7CX_LAST_THRESHOLD | thresholds[63].zone_num;
